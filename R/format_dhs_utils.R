...
get_births <- function(dat, 
  ...
  # convert exact_column to numeric just for this one bit of code
    suppressWarnings(age_in_years <- which(as.numeric(dat[, exact_age]) >= 300 & as.numeric(dat[, exact_age]) < 350))
    if (length(age_in_years) > 0) {
      suppressWarnings(age_in_years_vals <- as.numeric(dat[age_in_years, exact_age])-300)
